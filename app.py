...
def gconnect():
    ...
    try:
        auth_flow = flow_from_clientsecrets('client_secrets.json', scope='')
...
@app.route('/category/<int:category_id>/sport_item/')
def showsport_item(category_id):
    category = session.query(Category).filter_by(id=category_id).one()
    creator = getUserInfo(Category.user_id)
     items = session.query(SportItem).filter_by(
        catetgory_id=category_id).all()
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('catalog.html', items=items, category=category, creator=creator)
    else:
        return render_template('sport_item.html', items=items, category=category, creator=creator)
...
def newSportItem(category_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('showsport_item', category_id=category_id))
    else:
        return render_template('sportitem.html', category_id=category_id)


# Edit a sport item


@app.route('/category/<int:category_id>/sport_item/<int:sport_item_id>/edit', methods=['GET', 'POST'])
def editSportItem(category_id,):
    if 'username' not in login_session:
        return redirect('/login')
    editedItem=session.query(SportItem).filter_by(id=category_id).one()
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() " \
             "{alert('Unable to authorize sign in.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        ...
        return render_template('editsportitem.html', category_id=category_id, item=editedItem)

# Delete a menu item
@app.route('/category/<int:category_id>/sportitem/<int:sportitem_id>/delete', methods=['GET', 'POST'])
def deleteSportItem(category_id, sportitem_id):
    if 'username' not in login_session:
        return redirect('/login')
    category = session.query(Category).filter_by(id=category_id).one()
    itemToDelete = session.query(SportItem).filter_by(id=sportitem_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction()" \
